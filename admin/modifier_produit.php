...
if(!empty($_POST)) {
    ...
    if(empty($errors)) {
        ...
        if(!empty($_FILES)){
            $result = $connexion->prepare('UPDATE product SET name=:nom, price=:prix, category=:categorie, dispo=:disponibilite ');
        }else{
            $result = $connexion->prepare('UPDATE product SET name=:nom, price=:prix, category=:categorie, dispo=:disponibilite, photo = "' . $newName . '.' . $extension . '" ');
        }
                ...
                <div class="card-body">
                    <form method="post" action="modifier_produit.php" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" min="0" name="price" value="<?=$article['price'] ?>" class="form-control" >
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control" name="category"  id="exampleFormControlSelect1">
                                ...
                                foreach ($categories as $categorie){
                                    ?>
                                    <option value="<?=$categorie['id']?>" <?php if($categorie['id']==$article['category']){echo'selected';}?>><?=$categorie['category']?></option>
                                    <?php
                                }
                                ?>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <img class="card-img-top" src="../assets/img/<?=$article['photo']?>" alt="Image enregistré dans la bdd">
                            <input type="file" name="photo" class="form-control-file" id="exampleFormControlFile1">
                            <smal id="emailHelp" class="form-text text-muted">Taille max: 1Mo</smal>
                        </div>
                        <div class="form-group form-check">
                            <input type="checkbox"  name="dispo" <?php if($article['dispo']=='1'){echo'checked = true';} ?>class="form-check-input" id="exampleCheck1">
